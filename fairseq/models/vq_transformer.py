...
class VQTransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        #---------------------------------------------------------------------------------------------
        # args for Gumbel VQ
        parser.add_argument('--vq-num-vars', type=int, metavar='D', default=100,
                            help='size of a group for vector quantization')
        parser.add_argument('--vq-groups', type=int, metavar='D', default=1,
                            help='number of groups for vector quantization')
        parser.add_argument('--vq-gumbel-temp', type=str, metavar='D', default="(1, 0.1, 0.001)",
                            help='gumbel temperature for training. this should be a tuple of 3 elements: (start, stop, decay factor)')
        parser.add_argument('--vq-combine-groups', type=bool, metavar='D', default=False,
                            help='whether to use the vectors for all groups')
        parser.add_argument('--vq-loss-scaling', type=float, metavar='D', default=0.1,
...
class VQTransformerDecoder(FairseqIncrementalDecoder):
    """
    ...
    def extract_features_scriptable(
        ...
        output_dict = {"attn": [attn], "inner_states": inner_states}
...
@register_model_architecture("vq_transformer", "vq_transformer_iwslt_de_en")
def vq_transformer_iwslt_de_en(args):
